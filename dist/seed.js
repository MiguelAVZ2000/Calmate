...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { error: deleteProductsError } = yield supabase.from('products').delete().neq('id', 0);
        if (deleteProductsError) {
            console.error('Error limpiando la tabla products:', deleteProductsError);
            return;
        }
        const { error: deleteCategoriesError } = yield supabase.from('categories').delete().neq('id', 0);
        ...
        const { data: insertedCategories, error: insertCategoriesError } = yield supabase
            .from('categories')
            .insert(categories)
            .select();
        ...
        const productsToInsert = products.map(product => (Object.assign(Object.assign({}, product), { category_id: categoryMap[product.category_name], category_name: undefined })));
        // Insertar productos
        console.log('Insertando productos...');
        const { error: insertProductsError } = yield supabase.from('products').insert(productsToInsert);
