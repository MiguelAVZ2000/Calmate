...
export function ProductDetailsClient({ product, reviews }: { product: Product, reviews: Review[] }) {
  ...
  const handleAddToCart = () => {
    if (!isStockAvailable) {
      toast.error("No hay suficiente stock para la cantidad y peso seleccionados.");
      return;
    }
    ...
    toast.success(`${quantity} x ${product.name} (${selectedWeight}g) añadido(s) al carrito.`);
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center space-x-3 mb-6">
              ...
              {product.original_price && product.original_price > product.price && (
                <span className="font-serif text-2xl text-muted-foreground line-through">
                  {formatCurrency((product.original_price / BASE_WEIGHT) * selectedWeight)}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                {isStockAvailable ? `Añadir al Carrito - ${formatCurrency(calculatedPrice * quantity)}` : "Sin stock"}
              </Button>
            </div>
          </div>
